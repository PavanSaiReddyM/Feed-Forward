...
const ROLES = [
  { key: "Restaurant", emoji: "🍽️", label: "Restaurant", desc: "Share surplus meals", backendRole: "donor" },
  { key: "Hotel", emoji: "🏨", label: "Hotel / Catering", desc: "Donate event food", backendRole: "donor" },
  { key: "NGO", emoji: "🤝", label: "NGO / Charity", desc: "Receive & distribute", backendRole: "receiver" },
  { key: "Volunteer", emoji: "🚴", label: "Volunteer", desc: "Help with pickup", backendRole: "receiver" },
];
...
const sb = StyleSheet.create({
  ...
  circle: { width: 30, height: 30, borderRadius: 15, backgroundColor: "#EBEBF0", justifyContent: "center", alignItems: "center", zIndex: 1 },
...
function Field({ label, icon, value, onChangeText, placeholder, secure, keyboardType, right, valid }) {
  const [focused, setFocused] = useState(false);
  return (
    <View style={fld.group}>
      <Text style={[fld.label, focused && { color: COLORS.primary }]}>{label}</Text>
      <View style={[fld.box, focused && fld.boxFocused, valid && fld.boxValid]}>
        <MaterialCommunityIcons name={icon} size={18}
          color={valid ? COLORS.success : focused ? COLORS.primary : COLORS.grayText} />
        <TextInput
          style={fld.input} value={value} onChangeText={onChangeText}
          placeholder={placeholder} placeholderTextColor={COLORS.placeholder}
          secureTextEntry={secure} keyboardType={keyboardType}
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          autoCapitalize="none"
        />
        {right}
        {valid && !right && <MaterialCommunityIcons name="check-circle" size={18} color={COLORS.success} />}
      </View>
    </View>
  );
}
const fld = StyleSheet.create({
  ...
  box: { flexDirection: "row", alignItems: "center", backgroundColor: "#F7F7FA", borderRadius: 14, paddingHorizontal: 14, paddingVertical: 14, gap: 10, borderWidth: 1.5, borderColor: "transparent" },
...
function StrengthBar({ password }) {
  ...
  const colors = ["", "#EF4444", "#F59E0B", "#F4A261", COLORS.success];
...
export default function Signup() {
  ...
  const goNext = () => { setError(""); transition(1, () => setStep(s => s + 1)); };
  const goBack = () => { setError(""); transition(-1, () => setStep(s => s - 1)); };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await register({
        ...
        password,
        phone: phone.trim(),
        role: selectedRole?.backendRole ?? "donor",
      });
      // NGO / Volunteer must complete verification before accessing the app
      if (selectedRole?.backendRole === "receiver") {
        router.replace({
          pathname: "/verification",
          params: { name: name.trim(), email: email.trim().toLowerCase() },
        });
      } else {
  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      <View style={styles.nav}>
        <TouchableOpacity style={styles.navBack} onPress={step === 0 ? () => router.back() : goBack}>
        ...
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          {step === 0 && (
            <>
              <Field label="Full Name" icon="account-outline" value={name} onChangeText={setName}
                placeholder="John Doe" valid={name.length > 1} />
              <Field label="Email Address" icon="email-outline" value={email} onChangeText={setEmail}
                placeholder="you@example.com" keyboardType="email-address"
                valid={email.includes("@") && email.includes(".")} />
              <Field label="Phone Number" icon="phone-outline" value={phone} onChangeText={setPhone}
                placeholder="+91 98765 43210" keyboardType="phone-pad" valid={phone.length >= 8} />
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              <Field label="Confirm Password" icon="lock-check-outline" value={confirmPw} onChangeText={setConfirmPw}
      ...
      <View style={styles.bottom}>
        <TouchableOpacity
          style={[styles.nextBtn, (!canGoNext || loading) && styles.nextBtnOff]}
          onPress={step < 2 ? goNext : handleRegister}
          disabled={!canGoNext || loading}
          activeOpacity={0.86}
        >
          <Text style={styles.nextBtnText}>
            {step === 2 ? (loading ? "Creating Account…" : "Create Account") : "Continue"}
          </Text>
          <MaterialCommunityIcons name={step === 2 ? "check" : "arrow-right"} size={20} color="#fff" />
        </TouchableOpacity>
        {step === 0 && (
...
const styles = StyleSheet.create({
  ...
  nav: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, paddingTop: Platform.OS === "ios" ? 56 : 44, paddingBottom: 12 },
  navBack: { width: 40, height: 40, borderRadius: 12, backgroundColor: "#F5F5F8", justifyContent: "center", alignItems: "center" },
  ...
  rolesGrid: { gap: 12 },
  roleCard: { flexDirection: "row", alignItems: "center", gap: 14, backgroundColor: "#F7F7FA", borderRadius: 16, padding: 16, borderWidth: 1.5, borderColor: "transparent" },
  roleCardActive: { borderColor: COLORS.primary, backgroundColor: COLORS.primaryGlow, shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 10, elevation: 3 },
  roleEmojiBg: { width: 48, height: 48, borderRadius: 14, backgroundColor: "#EBEBF0", justifyContent: "center", alignItems: "center" },
  ...
  roleRadio: { width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: "#D0D0D8", justifyContent: "center", alignItems: "center" },
  roleRadioActive: { borderColor: COLORS.primary },
  roleRadioDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: COLORS.primary },
  summaryCard: { backgroundColor: COLORS.bg, borderRadius: 16, padding: 18, gap: 10, borderWidth: 1.5, borderColor: "rgba(0,0,0,0.07)", marginTop: 4 },
  summaryTitle: { fontSize: 11, fontWeight: "800", color: COLORS.grayText, textTransform: "uppercase", letterSpacing: 1, marginBottom: 2 },
  summaryRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  summaryVal: { fontSize: 14, fontWeight: "600", color: COLORS.textDark },
  errorBanner: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#FEF2F2", borderRadius: 12, padding: 12, marginTop: 8, borderWidth: 1, borderColor: "#FECACA" },
  errorText: { flex: 1, fontSize: 13, color: "#D32F2F", fontWeight: "600" },
  bottom: { paddingHorizontal: 22, paddingBottom: Platform.OS === "ios" ? 36 : 24, paddingTop: 16, backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: "#F0F0F5", gap: 12 },
  nextBtn: { backgroundColor: COLORS.primary, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, paddingVertical: 17, borderRadius: 16, shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.26, shadowRadius: 16, elevation: 6 },
